...
`(agent, runtime, policy, capability)` per 24-hour window. This is visibility,
not protection: the warning never refuses, blocks, or changes a job.
...
delta. This baseline intentionally lives in the Gitmoot home store, not in
source control or CI, because only that store contains the fleet inventory.
