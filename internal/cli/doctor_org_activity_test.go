...
func TestBuildOrgRoleActivityDoctorCheckClassifiesAbsence(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "stale target",
			presence: []db.OrgRolePresence{
				{Role: "owner", LastSeenAt: now.Add(-2 * time.Hour).Format(time.RFC3339)},
				{Role: "review", LastSeenAt: now.Add(-30 * time.Minute).Format(time.RFC3339)},
			},
			wantDetail: []string{"overdue: owner", "idle 2h0m0s", "recycle_after 1h"}, rejectDetail: "review (",
		},
		{
			name: "per-role override remains within policy",
			presence: []db.OrgRolePresence{
				{Role: "owner", LastSeenAt: now.Add(-time.Minute).Format(time.RFC3339)},
				{Role: "review", LastSeenAt: now.Add(-2 * time.Hour).Format(time.RFC3339)},
			},
			wantOK: true, wantDetail: []string{"all 2 monitored org roles"}, rejectDetail: "overdue: review",
		},
		{
			name: "never observed",
			presence: []db.OrgRolePresence{
				{Role: "review", LastSeenAt: now.Add(-time.Minute).Format(time.RFC3339)},
			},
			wantDetail: []string{"never observed: owner", "recycle_after 1h"},
		},
		{
...
func TestDoctorOrgRoleActivityWarnsAtOperatorArtifact(t *testing.T) {
	...
	seedOrgRolePresenceAt(t, paths, "owner", now.Add(-2*time.Hour), "stale")
	seedOrgRolePresenceAt(t, paths, "review", now.Add(-time.Minute), "fresh")
	...
	if check["status"] != "warn" || !strings.Contains(detail, "overdue: owner") || !strings.Contains(detail, "recycle_after 1h") || strings.Contains(detail, "review (") {
		t.Fatalf("org role activity artifact = %#v, want stale owner only with global policy", check)
	}
}
