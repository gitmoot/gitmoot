...
func TestImplementBranchReusePredicateStillRejectsPullRequestOpen(t *testing.T) {
	if taskBranchReusableForImplement(string(workflow.TaskPullRequestOpen)) {
		t.Fatal("implement branch-reuse predicate was widened to pr_open")
	}
	if taskBranchReusableForImplement(string(workflow.TaskDismissed)) {
		t.Fatal("implement branch-reuse predicate was widened to dismissed")
	}
}
