...
// TestWebDataSourceAgentConfig asserts the config-visibility enrichment: Agent()
// builds AgentDetail.Config from the agent's [agents.<name>] section — the seeded
// values as LoadAgentTypes resolves them, parse-time defaults folded in — and
// leaves Config nil when the agent has no section at all.
func TestWebDataSourceAgentConfig(t *testing.T) {
	home := dashboardTestHome(t)
	paths := config.PathsForHome(home)
	ctx := context.Background()

	// Config sections: full-agent sets every pool knob explicitly; plain-agent has
	// a section carrying nothing but the runtime (parse-time defaults fill the
	// rest); nosection-agent gets NO [agents.<name>] section at all.
	if err := config.SaveAgentType(paths, config.AgentType{
		Name: "full-agent", Runtime: "codex",
		MaxBackground: 3, IdleTimeout: "15m", JobTimeout: "30m",
		Model: "gpt-5.5", Template: "coordinator", Capabilities: []string{"orchestrate", "review"},
	}); err != nil {
		t.Fatalf("SaveAgentType full-agent: %v", err)
	}
	if err := config.SaveAgentType(paths, config.AgentType{Name: "plain-agent", Runtime: "claude"}); err != nil {
	...
	// registered, but only two carry a section.
	...
	// Agent(full-agent): the full config section, surfaced as seeded.
	full, err := ds.Agent(ctx, "full-agent")
	if err != nil {
		t.Fatalf("Agent(full-agent): %v", err)
	}
	if full.Config == nil {
		t.Fatalf("full-agent Config = nil, want a section")
	}
	if full.Config.MaxBackground != 3 || full.Config.IdleTimeout != "15m" ||
		full.Config.JobTimeout != "30m" || full.Config.Model != "gpt-5.5" || full.Config.Template != "coordinator" {
		t.Fatalf("full-agent Config = %+v, want the seeded section values", full.Config)
	}
	if len(full.Config.Capabilities) != 2 {
		t.Fatalf("full-agent Config.Capabilities = %v, want 2", full.Config.Capabilities)
	}

	// Agent(plain-agent): a section exists, with parse-time defaults folded in.
	plain, err := ds.Agent(ctx, "plain-agent")
	if err != nil {
		t.Fatalf("Agent(plain-agent): %v", err)
	}
	if plain.Config == nil {
		t.Fatalf("plain-agent Config = nil, want a non-nil section")
	}
	...
	// Agent(nosection-agent): no config section => nil Config.
