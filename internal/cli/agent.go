...
func printAgentUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  gitmoot agent subscribe <name> --runtime codex|claude|kimi|omp|shell --session <id|name|last|command> --role <role> [--repo owner/repo...] [--model model] [--effort effort] --capability <capability>")
...
func runAgentAsk(args []string, stdout, stderr io.Writer) int {
	...
	if !options.force && looksLikeWorkflowOrchestration(options.message) {
		...
		fmt.Fprintln(stderr, "Gitmoot does not dispatch implementation (#2203): implement in your own session and record it with `gitmoot job record --type implement`.")
	}
...
func parseAgentAskOptions(args []string, stderr io.Writer) (agentAskOptions, bool) {
	...
	if options.action != "" && options.action != "ask" {
		fmt.Fprintf(stderr, "agent ask: --action must be ask; use agent run to choose ask or review\n")
		return agentAskOptions{}, false
	}
...
// (#531) shared by agent ask/run/review and orchestrate. The valid
...
func printOrchestrateUsage(w io.Writer) {
	fmt.Fprintln(w, "Usage:")
	fmt.Fprintln(w, "  gitmoot orchestrate <agent> \"message\" [--repo owner/repo] [--pr number] [--head-sha sha] [--branch branch] [--type type] [--action ask|review] [--model model] [--effort effort] [--workflow id] [--recipe id] [--home path] [--json]")
	fmt.Fprintln(w)
...
func parseAgentRunOptions(command string, args []string, stderr io.Writer) (agentRunOptions, bool) {
	...
	for index := 0; index < len(args); index++ {
		...
		case strings.HasPrefix(arg, "--lead="):
			if !setAgentRunOption(&options, "--lead", strings.TrimPrefix(arg, "--lead="), stderr) {
				return agentRunOptions{}, false
			}
		case strings.HasPrefix(arg, "-") && len(positionals) >= 2:
	...
	// --lead is scoped to the local review surfaces. The shared parser would
	// otherwise accept it for implement/orchestrate and silently persist routing
	// metadata that those commands do not own. agent run is checked again after
	// action selection because its final action may come from flags or inference.
	if strings.TrimSpace(options.lead) != "" && command != "review" && command != "run" {
...
func setAgentRunOption(options *agentRunOptions, flagName string, value string, stderr io.Writer) bool {
	...
	case "--lead":
		if value == "" {
			fmt.Fprintln(stderr, "--lead requires a non-blank value")
			return false
		}
		options.lead = value
	case "--recipe":
...
func validateAgentRunActionOptions(command string, options agentRunOptions) error {
	...
	switch action {
	case "ask":
		if options.prNumber > 0 || strings.TrimSpace(options.headSHA) != "" || strings.TrimSpace(options.branch) != "" {
			return errors.New("--action ask cannot be combined with --pr, --head-sha, or --branch")
		}
	case "review":
		if options.prNumber <= 0 {
...
func printAgentRunUsage(w io.Writer, command string) {
	...
	switch command {
	case "orchestrate":
		fmt.Fprintln(w, "  gitmoot orchestrate <agent> \"message\" [--repo owner/repo] [--pr number] [--head-sha sha] [--branch branch] [--type type] [--action ask|review] [--model model] [--effort effort] [--workflow id] [--org-role role] [--runtime rt] [--session ref] [--recipe id] [--skip-native-review-fanout] [--home path] [--json]")
	case "review":
	...
	default:
		fmt.Fprintln(w, "  gitmoot agent run <name> \"message\" [--repo owner/repo] [--pr number] [--lead implementer] [--head-sha sha] [--branch branch] [--background|--foreground] [--type type] [--action ask|review] [--model model] [--effort effort] [--workflow id] [--org-role role] [--runtime rt] [--session ref] [--recipe id] [--skip-native-review-fanout] [--home path] [--json]")
	}
	printAgentRuntimeOverrideHelp(w)
}
...
func printAgentTypeUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  gitmoot agent type set <type> --runtime codex|claude|kimi|omp --template <template-id> --model <model> --effort <effort> --policy workspace-write --max-background 2 --idle-timeout 20m")
}
...
func runAgentTypeSet(args []string, stdout, stderr io.Writer) int {
	...
	fs.SetOutput(stderr)
	home := fs.String("home", "", "home directory to use instead of the current user's home")
	runtimeName := fs.String("runtime", "", "agent runtime: codex, claude, kimi, or omp")
	...
	if entry.Runtime == runtime.ShellRuntime {
		fmt.Fprintln(stderr, "invalid runtime: managed agent types support codex, claude, kimi, or omp")
		return 2
	}
...
func runAgentSubscribe(args []string, stdout, stderr io.Writer) int {
	...
	fs.SetOutput(stderr)
	home := fs.String("home", "", "home directory to use instead of the current user's home")
	runtimeName := fs.String("runtime", "", "agent runtime: codex, claude, kimi, omp, or shell")
