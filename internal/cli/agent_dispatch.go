...
func dispatchLocalAgentJob(ctx context.Context, store *db.Store, request localAgentDispatchRequest) (output localAgentJobOutput, err error) {
	...
	retainedLogFile, retainedLogErr := openRetainedTranscriptLog(request.Home, job.ID, job.Type, strings.TrimSpace(effectiveAgent.Runtime), job.LifecycleGeneration, store)
