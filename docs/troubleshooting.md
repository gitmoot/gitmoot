...
every entry with its inode and inode-change time. The removal then walks that
inventory with **descriptor-relative** operations — `openat`/`fstatat`/`unlinkat`
against directory descriptors this pass opened and verified — and unlinks only
objects that still match. Three things follow, all enforced by the kernel rather
than sampled by a check:

- content created after the proof is not in the inventory, is never unlinked, and
  makes its parent's `rmdir` fail with `ENOTEMPTY`, so the removal aborts and the
  clone is restored with the new bytes intact;
- a proved file rewritten with same-size content and a restored mtime is refused,
  because inode and ctime are compared and no unprivileged call moves ctime
  backwards;
- a directory swapped for another directory or a symlink cannot redirect anything,
  because no path component is re-resolved between the proof and the unlink.

Both names a writer could have observed are then **fenced**. A fence is a regular
file whose content carries a random **nonce recorded in the job's event log**
(`delegation_worktree_fence_written`); the public prefix alone proves nothing.
Anything else at a quarantine name — a directory, symlink, hard link, empty file,
same-length forgery, or a fence with no recorded nonce — is a SURVIVOR: never
deleted, never followed, and it blocks completion. Pruning re-proves ownership
through a descriptor and compares the inode again immediately before `unlinkat`,
so a name swapped after classification is not removed. Retired fences are swept on
the five-minute worktree-maintenance cadence, bounded to 256 entries per run with a
rotating cursor, and counted by `gitmoot doctor` and `/api/health`.
...
the daemon's allocation while valid large objects still stream. A pack must pass
its own trailing checksum, its `.idx` must verify and record that pack's digest,
and **`git verify-pack` must accept the pair** — checksums prove the files are
intact, only Git proves they hold readable objects. Ordinary ignored
content-addressed build output uses the same hex-fanout and `pack-<hash>.pack`
naming, so recognising on names alone would make the pass inert on any repo with a
build cache.

On non-Linux builds the removal and the fence prune FAIL CLOSED — nothing is
deleted — because the descriptor-relative primitives and ctime this proof depends
on are Linux-shaped, and a weaker stand-in is exactly what these guards replaced.
