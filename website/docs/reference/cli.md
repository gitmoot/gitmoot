...
(`off|minimal|low|medium|high|xhigh|max|auto`) — anything else is dropped, so a
typo falls back to omp's own default rather than silently downgrading the seat.
...
At job completion, the one winning sampled job for the coalescing window is
...
fallback, a branchless payload, or an unavailable answer.
`payload_had_pull_request` says only that the payload already carried a PR
number at capture; it does not attribute opening that PR to the sampled job.
This is effect measurement only: capture errors are logged and never change job
state or delivery.
...
the daemon runs under. What that changes in practice:

- Capabilities are `review`, `implement`, and `ask` — **not** `produce`.
- **Every job is a fresh session.** omp is stateless in v1 and never resumes,
  because `omp --resume` relocates the working directory to the previous
  session's cwd and overwrites `--cwd`: a resumed job would edit the *old*
  worktree and leave the job's own worktree clean, which surfaces as a green job
  with an empty diff. A `--session` ref may be a session UUID or
...
- **Authentication is per profile.** Authenticate omp once interactively, export
  ...
  see the binary — its `PATH` comes from the systemd EnvironmentFile, not a
  login shell.

Full behavioral contract (argument vector, retry semantics, usage source,
transcripts, curated environment): [Runtime
...
an enabled heartbeat pointed at omp spends whatever credential its profile
...
- **A dispatch that cannot be delegated RECORDS why.** No `--org-role` (the
  ...
  naming the reason — queryable months later with
  `gitmoot job show <id>`, not just printed. The unexpressible-flag case also
  prints to stderr; the other two do not, which is exactly why the durable event
  exists. A silently different review is worse than a refused one.
...
`recycle_after` duration override, and an optional `pane` Herdr binding. For
backward compatibility, a configured binding resolves as a literal pane id or a
unique exact live label. A literal id tracks one pane; a label tracks whichever
current pane uniquely carries that cosmetic value. `org seat add` canonicalizes
new bindings to ids. Roles without a binding report unknown live presence, and
event wakes for them are skipped with an observable log and increment the
role's missed-wake counter rather than being inferred from a pane label. There
...
quiet on healthy jobs. Failed checks are also stored durably for later
consumption as structured feedback; nothing consumes them today.
...
An enabled `trigger.kind: pipeline` pipeline fires once for each successful
upstream run. The durable cursor prevents duplicate fires across daemon restarts,
and adding or enabling the downstream arms it without backfilling old history.
Missing upstreams remain dormant. Self-reference, cycles, and schedule-plus-
trigger hybrids are rejected.
