...
Start an agent against an installed template — the built-in thermo review
template, say:
...
`task_blocked_job_failed`), and job-retry recovery.
...
context that Gitmoot cannot reconstruct. When enabled it uses the
